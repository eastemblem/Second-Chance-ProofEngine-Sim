...
- **ProofCoach Intelligent Guidance System**: Dual-mode AI-based coaching system with (1) Tutorial Mode for anonymous onboarding and (2) Coach Mode for authenticated journey tracking. **Tutorial Mode** provides contextual, persistent coaching across all onboarding pages, working pre-authentication with client-side localStorage tracking. Features auto-triggered tutorials that respect page-level completion state using `tutorialCompletedPages` tracking to prevent re-triggering. Comprehensive tutorials implemented for all pages: Founder (9 steps), Venture (9 steps), Team (4 steps), Upload (4 steps), Analysis (5 steps), Dashboard (9 steps), and Validation Map (6 steps). **Coach Mode** features post-authentication journey tracking with 7 milestone-based steps (IDs 10-16): complete experiments (3+), export validation map CSV, upload to ProofVault, reach upload thresholds, achieve ProofScore 70+, complete payment, and access unlocked features. **Progress Tracking Architecture**: Backend `/api/v1/coach/progress` endpoint aggregates completion metrics via parallel COUNT queries (experiments, vault uploads, ProofScore, deal room access) using `DatabaseService.getCoachProgress()`. React Query hook `useCoachProgress` with 5-minute cache and refetchOnWindowFocus enabled. ProofCoachContext merges server-side metrics with founderId-namespaced localStorage flags, exposing `ventureProgress`, `refreshVentureProgress()`, `isStepCriteriaMetByBackend()`, `markCsvExported()`, and `markCommunityAccessed()` helpers. VentureProgressData interface covers all journey step checkField values with type-safe completion validation. Purple gradient styling (from-purple-700/95 via-fuchsia-600/90 to-indigo-700/95) with "Tutorial" badge branding (from-purple-500 to-fuchsia-500). **Congratulatory Messages**: Each stage begins with congratulations for completing the previous step. **Integration Points**: ValidationMap calls markCsvExported on export, Community/Downloads call markCommunityAccessed on access, mutations call refreshVentureProgress after success.
