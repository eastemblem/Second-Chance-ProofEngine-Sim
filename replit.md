...
- **ProofCoach Tutorial System**: Intelligent AI-based guidance system providing contextual, persistent coaching across all onboarding pages. Features auto-triggered tutorials that work pre-authentication, track progress client-side in localStorage, and respect page-level completion state using `tutorialCompletedPages` tracking to prevent re-triggering on page revisits. Comprehensive tutorials implemented for all onboarding pages: Founder (9 steps), Venture (9 steps covering industry, geography, business model, revenue/product stages, website, description, social media), Team (4 steps), Upload (4 steps), Analysis (5 steps covering ProofScore, ProofTags, report downloads, and next steps), and Dashboard (9 steps with welcome congratulations). Purple gradient styling matches dashboard design (from-purple-700/95 via-fuchsia-600/90 to-indigo-700/95). Tutorial auto-start logic uses `autoStart={!tutorialCompletedPages.includes(currentPageKey)}` to ensure tutorials trigger once per page and respect dismissals. **Congratulatory Messages**: Each onboarding stage begins with congratulations for completing the previous step (Venture congratulates founder profile completion, Team congratulates venture details, Upload congratulates team setup, Dashboard welcomes and congratulates complete onboarding). **Branding**: Tutorial overlay displays "Proof Coach" branding with a purple gradient "Tutorial" badge (from-purple-500 to-fuchsia-500) for visual consistency and clear mode identification.
