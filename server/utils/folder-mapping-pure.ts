...
/**
 * 100% DATABASE-DRIVEN FOLDER MAPPING - NO FALLBACKS OR CACHING
 * All folder IDs must come from proof_vault table populated during onboarding
 */
 ...
 */
export async function getCategoryFromFolderId(folderId: string, founderId: string): Promise<string> {
  const mapping = await loadFolderMappingFromDatabase(founderId);
...
/**
 * Get all available categories for a founder
 * @param founderId - Current user's founder ID
 * @returns Promise with array of available categories
 */
export async function getAvailableCategories(founderId: string): Promise<string[]> {
  const mapping = await loadFolderMappingFromDatabase(founderId);
  return mapping.availableCategories;
}
...
/**
 * Validate that all required proof categories exist for a founder
 * @param founderId - Current user's founder ID
 * @returns Promise with validation result
 */
export async function validateProofVaultStructure(founderId: string): Promise<{
  valid: boolean;
  ...
  try {
    const mapping = await loadFolderMappingFromDatabase(founderId);
    const missingCategories = requiredCategories.filter(cat => !mapping.categoryToFolderId[cat]);
    
    return {
      valid: missingCategories.length === 0,
      missingCategories,
      availableCategories: mapping.availableCategories
    };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    appLogger.database('Error validating proof vault structure', { founderId, error: errorMessage });
    return {
      valid: false,
